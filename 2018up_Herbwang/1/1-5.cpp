...
while(1)
{
cout<<"（hermite插值法）请输入插值节点的间隔"<<endl;
cin>>sh;
n=1/sh+2;         //保证至少有三个节点
h=new double[n];  //动态
...
{if(x5[i1]<=1.5*sh)
	...
	{x3[0]=x3[1];x3[1]=x3[2];}
	while (i3<3)
	{df3[i3]=(-50*x3[i3])/((1+25*x3[i3]*x3[i3])*(1+25*x3[i3]*x3[i3]));
	i3++;}i3=0;
		s2=(1+(2*(x5[i1]-x3[0]))/(x3[1]-x3[0]))
			*((x5[i1]-x3[1])/(x3[0]-x3[1]))
			*((x5[i1]-x3[1])/(x3[0]-x3[1]));
	s3=(1+(2*(x5[i1]-x3[1]))/(x3[0]-x3[1]))
			*((x5[i1]-x3[0])/(x3[1]-x3[0]))
			*((x5[i1]-x3[0])/(x3[1]-x3[0]));
	s4=(x5[i1]-x3[0])*((x5[i1]-x3[1])/(x3[0]-x3[1]))
			*((x5[i1]-x3[1])/(x3[0]-x3[1]));
	s5=(x5[i1]-x3[1])*((x5[i1]-x3[0])/(x3[1]-x3[0]))
			*((x5[i1]-x3[0])/(x3[1]-x3[0]));
		p5[i1]=f3[0]*s2+f3[1]*s3+df3[0]*s4+df3[1]*s5;
	 f5[i1]=1/(1+25*x5[i1]*x5[i1]);
