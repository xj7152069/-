...
double G[M][N]={0.0},T[M][1]={0.0},T2[M][1]={0.0},DT[M][1]={0.0},GT[N][M]={0.0},GTG[N][N]={0.0},G_1[N][M]={0.0},U[N][N]={0.0},UT[N][N]={0.0},AU[N][N]={0.0},K_1[N][N]={0.0},K_1_GT[N][M]={0.0},model[N][1]={0.0},P[N][1]={0.0},GTd[N][1]={0.0},raypath[9999][3]={-1.0};
...
int main()
{
     double x1,x2,y1,y2,sx_a,sx_b,Lx1,Lx2,Ly1,Ly2,I,eps,eps1,ddbc;
     double maxdm,mindm;
     ifstream infile;
     ofstream outf;
...
///////////读入初始模型/////////////////////////////////
     ...
     {for(j=0;j<Z;j++)
      {
      infile.read(( char *)&model2[i][j],sizeof(model2[i][j]));  
       }
      }
     infile.close();
...
system("mpirun -np 4 ./first_raytracing_liu \n");
//////////////////////////////////////////////////////
while(eps>0)
{ 
  G3=-1;G1=-1;
...
/*    for(i=0;i<M;i++)
  ...
  {for(i=0;i<X;i++)
   ...
   {for(j=0;j<Z;j++)
    {   
        model1[i][j]=modeldd[i][j]+(P[X*(j)+i][0]);
...
break;
  ...
  for(i=0;i<X;i++)
    {for(j=0;j<Z;j++)
      {
       outf.write((char*)&modeldd[i][j],sizeof(modeldd[i][j]));
       }
    }
   outf.close(); ///   
