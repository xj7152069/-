...
using namespace std;
float p2[2000][2000];
float *t,**s1,**s2,**s3,**s4,**sx11,**sx12,**sx13,**sx21,**sx22,**sx23,**sx24,**sx31,**sx32,**sx33;
int main()
{   
     ifstream infile;
int X1,Y1,k,k1,X,T,i,j,i1,j1,Y,i2,j2,js(0);
float a2,a1,fy1,fy2,fy3,v,dt,pydx,jd,sjyc;
sjyc=-0.0,a2=0;
   float DX,DY,DT,dx,dy,ddx,ddy,snx1,sny1,snx2,sny2,t2,t5;
    int n,n1,t3,t4;
    ...
    float u0(0),u1(0),u2(0),u3(0),u4(0),u5(0),u(0),ux(0),uy(0);

cout<<"please save the data as 'stack.bin' "<<endl;
...
cin>>T;
 ...
 cout<<"do you wannt to use the model?(input 1 to use)"<<endl;
 cin>>a2;
if(a2 !=1)
...
else
{
    ...
    {cout<<"the Y has over the range, please input again"<<endl;
     ...
     if(!infile) cout<<"error"<<endl;    //输入速度模型
      for(j=0;j<X;j++)
     {for(i=0;i<Y;i++)
      {
...
cin>>jd;
cout<<"input '1' to output the movie, input other number to cancel"<<endl;
cin>>a1;
...
vector<vector<float> > f(T,vector<float>(X));
vector<vector<float> > r(T,vector<float>(X));
vector<vector<float> > ss(Y,vector<float>(X));

     float C3=5.0/2/(30)/DX*log(1000)/(30)/DX/(30)/DX;
     t5=double(Y)/X;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     infile.open("stack.bin",ios::binary);
     if(!infile) cout<<"error"<<endl;
      for(i=0;i<X;i++)
     {for(j=0;j<T;j++)
      {infile.read(( char *)&s[j][i],sizeof(s[j][i]));
       }
      }
     infile.close();
...
if(j==40 )
   {for(i1=1;i1<k1;i1++)
     {f[k][i-5]=f[k][i-5]+pow(s[T-i1][i-5]*exp((-3.1415*3.1415*900*(t[i1]*DT-sjyc)*(t[i1]*DT-sjyc)))*(3-2*3.1415*3.1415*900*(t[i1]*DT-sjyc)*(t[i1]*DT-sjyc)),jd);   
     }
   }
if(j==40 )
     {f[k][i-5]=f[k][i-5]+pow(s[T-k][i-5]*exp((-3.1415*3.1415*900*(DT-sjyc)*(DT-sjyc)))*(3-2*3.1415*3.1415*900*(DT-sjyc)*(DT-sjyc)),jd);}
...
js=js+1;
           ...
           if(j==40 )
           {s3[j][i]=(u1+u2-2*u*s2[j][i])*u3+2*s2[j][i]-s1[j][i]+f[k][i-5];}
           else
           {s3[j][i]=(u1+u2-2*u*s2[j][i])*u3+2*s2[j][i]-s1[j][i];}
           snx1=0.0;snx2=0.0;
           sny1=0.0;sny2=0.0;
...
sx31[j][i]=DT*DT*p2[j][i]*p2[j][i]*(1.0/(DX*DX))*(u2-u*s2[j][i])+2*sx32[j][i]-sx33[j][i];}     
     ...
     // if(snx1!=0 || snx2!=0 || sny1!=0 || sny2!=0)
        if(j==40)
        {s3[j][i]=sx11[j][i]+sx21[j][i]+sx31[j][i]+f[k][i-5];}
        else
        {s3[j][i]=sx11[j][i]+sx21[j][i]+sx31[j][i];}

        if(k%1==0 && a1==1)
        {outf1.write((char*)&s3[j][i],sizeof(s3[j][i]));}
...
outf1.close();
////////////////////////////////////////////////////////////////////
     cout<<"the data has been saved as 'nspy' "<<endl;

   if(a1==1)
     { cout<<"the movie has been saved as 'ns_movie' "<<endl;}
return 0;
}
