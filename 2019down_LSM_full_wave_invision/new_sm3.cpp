...
**sx22,**sx23,**sx24,**sx31,**sx32,**sx33,**suface_data,*zbbx,**nspy,**nspy2;
...
///////////////////////输入参数///////////////////////////////////
    ...
    {strcpy(str_suface,"asuface");
     strcpy(str_movie,"amovie");
     strcpy(str_forwave,"aforwave");
    }
    strcpy(str_nspy,"nspy_source");
...
/*在逆时偏移情况下，读入地表记录*/
    ...
    for(j=0;j<X;j++)
     {for(i=2;i<T-2;i++)
      {
        nspy[i][j]=(1.0/12.0/DT/DT)*(-nspy2[i-2][j]+16*nspy2[i-1][j]-30*nspy2[i][j]+16*nspy2[i+1][j]-nspy2[i+2][j]);
       }
      }*/
